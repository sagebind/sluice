...
pub fn new(count: usize) -> (Reader, Writer) {
    let (mut buf_pool_tx, buf_pool_rx) = crossbeam_channel::bounded(count);
    let (buf_stream_tx, buf_stream_rx) = crossbeam_channel::bounded(count);

    let buf_pool_waker = Arc::new(AtomicWaker::new());
    let buf_stream_waker = Arc::new(AtomicWaker::new());
    ...
    let reader = Reader {
        buf_pool_tx,
        buf_pool_waker: buf_pool_waker.clone(),
        buf_stream_rx,
        buf_stream_waker: buf_stream_waker.clone(),
        chunk: None,
    };
    ...
    let writer = Writer {
        buf_pool_rx,
        buf_pool_waker,
        buf_stream_tx: Some(buf_stream_tx),
        buf_stream_waker,
    };
        ...
        let mut chunk = match self.chunk.take() {
            ...
            None => match self.buf_stream_rx.try_recv() {
                ...
                Err(crossbeam_channel::TryRecvError::Empty) => {
                    self.buf_stream_waker.register(cx.waker());
                    return Poll::Pending;
                }

                // Pipe has closed, so return EOF.
                Err(crossbeam_channel::TryRecvError::Disconnected) => return Poll::Ready(Ok(0)),
...
pub struct Writer {
    ...
    buf_stream_tx: Option<crossbeam_channel::Sender<Vec<u8>>>,
    buf_stream_waker: Arc<AtomicWaker>,
}
...
impl AsyncWrite for Writer {
    fn poll_write(mut self: Pin<&mut Self>, cx: &mut Context, buf: &[u8]) -> Poll<io::Result<usize>> {
        ...
        match self.buf_pool_rx.try_recv() {
            ...
            Err(crossbeam_channel::TryRecvError::Empty) => {
                self.buf_pool_waker.register(cx.waker());
                Poll::Pending
            },

            // Pipe has closed.
            Err(crossbeam_channel::TryRecvError::Disconnected) => Poll::Ready(Err(io::ErrorKind::BrokenPipe.into())),
            ...
            Ok(mut chunk) => {
                ...
                match self.buf_stream_tx.as_mut().unwrap().try_send(chunk) {
                    Ok(()) => {
                        self.buf_stream_waker.wake();
